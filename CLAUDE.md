...
- **Per-scene dispatch** (`process_notebooklm`, 1.6.0+): `SceneDetector::detect_boundaries` splits the video; per scene a **presence gate** (`NotebookLMDetector::mark_present_in_scene` — template match on sampled frames, conf ≥ `notebooklm_presence_threshold`) and a **complexity gate** (`background_complexity_score` in `video/notebooklm_gates.{hpp,cpp}` — Sobel gradient-energy in a gapped band around the mark) decide: absent → write through unmodified; present → NS inpaint on a padded ROI crop (`inpaint_mark_roi`). Single-file output (NOT split like the Gemini `--scenes` branch); audio copied once.
...
- **CLI**: `wmr video in.mp4 -o out.mp4 --notebooklm` (auto-detect); `--rect x,y,w,h` manual override; `--notebooklm-method {ns|shiftmap|lama}`; `--complexity-threshold`. Config: `VideoWatermarkConfig::{notebooklm_rect, notebooklm_method, notebooklm_complexity_threshold, notebooklm_presence_threshold}`.
- **Tiers**: NS (current). Phase B: clean-frame copy (use mark-absent frames as real-bg source) + `cv::xphoto::inpaint INPAINT_SHIFTMAP` (opencv_contrib, gated `WMR_HAS_XPHOTO` — note xphoto's mask is **inverted** vs `cv::inpaint`, and SHIFTMAP expects CIELab). Phase C (optional, `WMR_BUILD_AI_LAMA`): LaMa ONNX (ONNX Runtime = a second ML runtime; heavy). SHIFTMAP on a small thin hole is **empirically unverified** — A/B before shipping Phase B.
- **Known limitation**: on complex/textured backgrounds (explainer mode) NS fabricates the region from its boundary — usable, not perfect. Phase B is the intended fix.
